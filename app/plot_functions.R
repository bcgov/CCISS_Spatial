...
plot_bgc <- function(dbCon, studyarea, xvariable, gcm_nm, run_nm, unit = c("Zone","Subzone"), focal_bgc = NULL){
...
plot_species <- function(dbCon, studyarea, xvariable,  gcm_nm, run_nm, edatope, spp_select = NULL, focal_species = NULL){
  ...
  if(!is.null(focal_species)){
    ...
    ggobj <- ggplot() +
      geom_line_interactive(data = dat_spline, aes(x = xval, y = area, group = spp, linetype = ltype, data_id = spp, tooltip = spp), color = "grey") +
    ...
    ggobj <- ggplot(dat_spline, aes(x = xval, y = area, col = spp, data_id = spp, tooltip = spp)) +
      geom_line_interactive(aes(linetype = ltype)) +
      scale_linetype_identity() +
      geom_text_repel(data = dat_ends, aes(x = xval, y = area, label = spp)) +
      geom_point(data = spp_obs, aes(x = xvar, y = area, col = spp)) +
      theme_bw() +
